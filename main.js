Swal.fire({
    title: "Bienvenido al Simulador de Ahorro con Interés y Tabla de Crecimiento",
    width: 600,
    padding: "3em",
    color: "#716add",
    background: "#fff url(/images/ahorro.gif)",
    backdrop: `
      rgba(0,0,123,0.4)
      url("/images/ahorros.gif")
      left top
      no-repeat
    `
  });
...
document.getElementById("ahorroForm").addEventListener("submit", function (event) {
    ...
    let montoInicial = parseFloat(
      document.getElementById("montoInicial").value
    );
    let tasaInteresAnual = parseFloat(
    ...
    document.getElementById(
      "resultado"
    ).innerHTML = `El valor futuro del ahorro después de ${anios} años será: $${valorFuturo}`;
    ...
    tablaCrecimientoHTML += "</table>";
    document.getElementById("tablaCrecimiento").innerHTML =
      tablaCrecimientoHTML;
  });
